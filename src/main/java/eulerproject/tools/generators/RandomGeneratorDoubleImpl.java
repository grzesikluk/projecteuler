...
public class RandomGeneratorDoubleImpl<T> implements RandomGenerator<T>
{
    ...
    public RandomGeneratorDoubleImpl(final Set<T> initSet) {
    ...
    public RandomGeneratorDoubleImpl(final Map<T,Double> map) {
    ...
    private boolean isProbabilitiesMapCorrect(final Map<T,Double> probabilityMap) {
        return Math.abs(probabilityMap.values().stream().mapToDouble(d->d).sum()-1.0)< 0.000000001;
    }
